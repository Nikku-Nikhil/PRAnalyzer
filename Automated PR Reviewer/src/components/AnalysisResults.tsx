...
export function AnalysisResults({ result }: AnalysisResultsProps) {
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      case 'style': return <FileText className="h-4 w-4" />
  ...
  const infoCount = result.issues.filter(issue => issue.type === 'info').length
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>{result.title} by {result.author}</CardDescription>
            </div>
          ...
          <div className="space-y-4">
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <span className={getScoreColor(result.metrics.codeQuality)}>
                  {result.metrics.codeQuality}%
                </span>
              </div>
              <Progress value={result.metrics.codeQuality} className="h-2" />
            </div>
            <div>
              <div className="flex justify-between items-center mb-2">
                <span className="flex items-center gap-2">
                ...
                <span className={getScoreColor(result.metrics.security)}>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <span className={getScoreColor(result.metrics.performance)}>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                ...
                <span className={getScoreColor(result.metrics.maintainability)}>
        ...
        <CardContent>
          <Tabs defaultValue="all" className="w-full">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant={issue.severity === 'high' ? 'destructive' : issue.severity === 'medium' ? 'outline' : 'secondary'}>
                          {issue.severity}
                        </Badge>
                      </div>
                      <AlertDescription>
            ...
            <TabsContent value="error" className="space-y-3 mt-4">
              {result.issues.filter(issue => issue.type === 'error').map((issue) => (
            ...
            <TabsContent value="warning" className="space-y-3 mt-4">
              {result.issues.filter(issue => issue.type === 'warning').map((issue) => (
                <Alert key={issue.id} className="p-4">
                  <div className="flex items-start gap-3">
                    {getIssueIcon(issue.type)}
                    <div className="flex-1 space-y-2">
            ...
            <TabsContent value="info" className="space-y-3 mt-4">
              {result.issues.filter(issue => issue.type === 'info').map((issue) => (
                <Alert key={issue.id} className="p-4">
                  <div className="flex items-start gap-3">
                    {getIssueIcon(issue.type)}
                    <div className="flex-1 space-y-2">
